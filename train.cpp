...
// 保存模型到文件
void model::store(string store_path)
{
    ofstream out(store_path);
    if (!out.is_open()) {
        cerr << "Cannot open file for writing: " << store_path << endl;
        return;
    }
    // 保存preterminals
    out << preterminals.size() << endl;
    for (auto &pt : preterminals) {
        out << pt.content.size();
        for (auto &seg : pt.content) {
            out << " " << seg.type << " " << seg.length;
        }
        out << endl;
    }
    // 保存preterm_freq
    out << preterm_freq.size() << endl;
    for (auto &kv : preterm_freq) {
        out << kv.first << " " << kv.second << endl;
    }
    // 保存letters
    out << letters.size() << endl;
    for (auto &seg : letters) {
        out << seg.type << " " << seg.length << " " << seg.values.size() << endl;
        for (auto &kv : seg.values) {
            out << kv.first << " " << seg.freqs.at(kv.second) << endl;
        }
    }
    // 保存digits
    out << digits.size() << endl;
    for (auto &seg : digits) {
        out << seg.type << " " << seg.length << " " << seg.values.size() << endl;
        for (auto &kv : seg.values) {
            out << kv.first << " " << seg.freqs.at(kv.second) << endl;
...
// 从文件加载模型
void model::load(string load_path)
{
    ifstream in(load_path);
    if (!in.is_open()) {
        cerr << "Cannot open file for reading: " << load_path << endl;
        return;
    }
    preterminals.clear();
    preterm_freq.clear();
    ...
    for (int i = 0; i < n; ++i) {
        ...
        for (int j = 0; j < m; ++j) {
            int t, l;
            in >> t >> l;
            pt.content.emplace_back(segment(t, l));
        }
